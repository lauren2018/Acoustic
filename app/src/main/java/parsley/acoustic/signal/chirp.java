package parsley.acoustic.signal;

import android.content.Context;

import java.util.ArrayList;
import java.util.Map;

import parsley.acoustic.view.basic.Param;

public class chirp {
    static float [] y;
    static float phase;


    public static float[] get_chirp(float [] t,float f0, float t1, float f1, float initPhase ){
        float [] f = new float[t.length];
        y = new float[t.length];
        float integral = 0;
        int i = 0;
        for(float x: t){
            float tmpf = f0 + (f1 - f0) * x / t1;
            integral = f0 * x + x*x*(f1-f0) / (2*t1);
            f[i] = tmpf;
            y[i] = (float)Math.cos(2*Math.PI*integral+initPhase);
            i++;
        }
        phase = (float)(2*Math.PI*integral+initPhase);
        return y;
    }

    public float[] get_chirp(float [] t, float f0, float df, float initPhase){
        float [] f = new float[t.length];
        y = new float[t.length];
        float integral = 0;
        int i = 0;
        for(float x: t){
            float tmpf = f0 + df * i;
            integral = f0 * x + x*x*df / 2;
            f[i] = tmpf;
            y[i] = (float)Math.cos(2*Math.PI*integral+initPhase);
            i++;
        }
        return y;
    }

    public float[] get_chirp(float [] t, int begin, int end, float f0, float df, float initPhase){
        float [] f = new float[t.length];
        y = new float[t.length];
        float integral = 0;
        for(int i = begin; i < end;i++){
            float x = t[i];
            float tmpf = f0 + df * i;
            integral = f0 * x + x*x*df / 2;
            f[i] = tmpf;
            y[i] = (float)Math.cos(2*Math.PI*integral+initPhase);
            i++;
        }
        return y;
    }

    public static float get_phase(){
        return phase;
    }
}
